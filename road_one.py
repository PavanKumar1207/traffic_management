...
video_path = '5229647-uhd_3840_2160_30fps.py'
...
columns = ['Frame', 'Zone A Count', 'Zone B Count', 'Zone C Count']
...
while True:
    ...
    # Update vehicle count and write to CSV
    vehicle_data = vehicle_data.append({
        'Frame': frame_number,
        'Zone A Count': len(zoneAcounter),
        'Zone B Count': len(zoneBcounter),
        'Zone C Count': len(zoneCcounter)
    }, ignore_index=True)
    ...
    # Save data to CSV periodically (e.g., after every 30 frames)
    if frame_number % 30 == 0:
        vehicle_data.to_csv('vehicle_counts.csv', index=False)
